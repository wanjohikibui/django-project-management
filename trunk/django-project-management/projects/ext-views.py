...
def get_users(request, project_number):
	...
	return HttpResponse( serializers.serialize('json', User.objects.filter(active=True, groups__in=project.read_acl.all()).distinct(), excludes=('is_active', 'is_superuser', 'is_staff', 'last_login', 'groups', 'user_permissions', 'password', 'email', 'date_joined') ))
...
def get_non_team_managers(request, project_number):
	...
	return HttpResponse( serializers.serialize('json', User.objects.filter(active=True, groups__in=project.read_acl.all()).exclude(id__in=project.team_managers.all()), excludes=('is_active', 'is_superuser', 'is_staff', 'last_login', 'groups', 'user_permissions', 'password', 'email', 'date_joined')))
